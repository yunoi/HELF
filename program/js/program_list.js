function pick_insert(o_key){
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      // var html = "<button type='button' id='$o_key' class='cancel_pick' onclick=\"pick_delete('$o_key')\">이미찜</button><br>";
      // $(".buttons").append(html);
...
function pick_delete(o_key){
  ...
  $.ajax({
    ...
    success:function(data){
      // document.getElementById(sokey).parentNode.removeChild(document.getElementById(sokey));
      // var html = "<button type='button' id='$o_key' class='btn_pick' onclick=\"pick_insert('$o_key');\">찜하기</button><br>";
      // $(".buttons").append(html);
